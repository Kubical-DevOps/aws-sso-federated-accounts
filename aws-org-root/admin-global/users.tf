...
module "admins_group" {
  ...
  group_name = "admins"
  allowed_roles = [
    "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/admin-root-org",
...
resource "aws_iam_role_policy_attachment" "billing_access" {
  role       = "billing"
  policy_arn = aws_iam_policy.limited_billing_access.arn
}
...
module "billing_role_access" {
  ...
  source_account_id = aws_organizations_account.kubical_aws_sso.id
}
